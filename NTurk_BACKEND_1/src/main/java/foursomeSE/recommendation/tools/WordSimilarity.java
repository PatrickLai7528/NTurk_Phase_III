...
public class WordSimilarity {
    ...
    // 静态代码块 Read词林必须在所有函数执行之前进行
    // 如果把readCiLin()放到getSimilarity中，可能会多次被执行浪费时间
    static {
        readCiLin();
    }
    ...
    private static void readCiLin() {
        InputStream input = WordSimilarity.class.getClassLoader().getResourceAsStream("data/directory/cilin.txt");
     ...
     * 根据word的内容，返回其对应的编码
     */
    private static ArrayList<String> getEncode(String word) {
     ...
     * @return 经过计算之后得到N的值
     */
    private static int getN(String encodeHead) {
    ...
    private static int getCount(String encodeHead, int end) {
     ...
     */
    private static int getK(String encode1, String encode2) {
     ...
     * @return
     */
    private static String getCommonStr(String encode1, String encode2) {
